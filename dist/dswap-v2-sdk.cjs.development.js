 ...
 */
var FACTORY_ADDRESS = '0x5D370a6189F89603FaB67e9C68383e63F7B6A262';
var FACTORY_ADDRESS_MAP = dswapSdkCore.V2_FACTORY_ADDRESSES;
var INIT_CODE_HASH = '0x7240a7a71cc85d56ee2e5ca1c5236ce877abc9b49b4696adc94968ef43796fed';
...
var ONE_HUNDRED_PERCENT = /*#__PURE__*/new dswapSdkCore.Percent(ONE);
...
var Pair = /*#__PURE__*/function () {
  function Pair(currencyAmountA, tokenAmountB) {
    ...
    this.liquidityToken = new dswapSdkCore.Token(tokenAmounts[0].currency.chainId, Pair.getAddress(tokenAmounts[0].currency, tokenAmounts[1].currency), 18, 'UNI-V2', 'Uniswap V2');
  ...
  _proto.getOutputAmount = function getOutputAmount(inputAmount, calculateFotFees) {
    ...
    var inputAmountAfterTax = percentAfterSellFees.greaterThan(ZERO_PERCENT) ? dswapSdkCore.CurrencyAmount.fromRawAmount(inputAmount.currency, percentAfterSellFees.multiply(inputAmount).quotient // fraction.quotient will round down by itself, which is desired
    ) : inputAmount;
    ...
    var outputAmount = dswapSdkCore.CurrencyAmount.fromRawAmount(inputAmount.currency.equals(this.token0) ? this.token1 : this.token0, JSBI.divide(numerator, denominator) // JSBI.divide will round down by itself, which is desired
    );
    if (JSBI.equal(outputAmount.quotient, ZERO)) {
      throw new InsufficientInputAmountError();
    }
    var percentAfterBuyFees = calculateFotFees ? this.derivePercentAfterBuyFees(outputAmount) : ZERO_PERCENT;
    var outputAmountAfterTax = percentAfterBuyFees.greaterThan(ZERO_PERCENT) ? dswapSdkCore.CurrencyAmount.fromRawAmount(outputAmount.currency, outputAmount.multiply(percentAfterBuyFees).quotient // fraction.quotient will round down by itself, which is desired
    ) : outputAmount;
  ...
  _proto.getInputAmount = function getInputAmount(outputAmount, calculateFotFees) {
    ...
    var percentAfterBuyFees = calculateFotFees ? this.derivePercentAfterBuyFees(outputAmount) : ZERO_PERCENT;
    var outputAmountBeforeTax = percentAfterBuyFees.greaterThan(ZERO_PERCENT) ? dswapSdkCore.CurrencyAmount.fromRawAmount(outputAmount.currency, JSBI.add(outputAmount.divide(percentAfterBuyFees).quotient, ONE) // add 1 for rounding up
    ) : outputAmount;
    ...
    var inputAmount = dswapSdkCore.CurrencyAmount.fromRawAmount(outputAmount.currency.equals(this.token0) ? this.token1 : this.token0, JSBI.add(JSBI.divide(numerator, denominator), ONE) // add 1 here is part of the formula, no rounding needed here, since there will not be decimal at this point
    );
    var percentAfterSellFees = calculateFotFees ? this.derivePercentAfterSellFees(inputAmount) : ZERO_PERCENT;
    var inputAmountBeforeTax = percentAfterSellFees.greaterThan(ZERO_PERCENT) ? dswapSdkCore.CurrencyAmount.fromRawAmount(inputAmount.currency, JSBI.add(inputAmount.divide(percentAfterSellFees).quotient, ONE) // add 1 for rounding up
    ) : inputAmount;
  ...
  _proto.getLiquidityMinted = function getLiquidityMinted(totalSupply, tokenAmountA, tokenAmountB) {
    ...
    if (JSBI.equal(totalSupply.quotient, ZERO)) {
      liquidity = JSBI.subtract(dswapSdkCore.sqrt(JSBI.multiply(tokenAmounts[0].quotient, tokenAmounts[1].quotient)), MINIMUM_LIQUIDITY);
    } else {
    ...
    return dswapSdkCore.CurrencyAmount.fromRawAmount(this.liquidityToken, liquidity);
  };
  _proto.getLiquidityValue = function getLiquidityValue(token, totalSupply, liquidity, feeOn, kLast) {
    ...
    if (!feeOn) {
      ...
      if (!JSBI.equal(kLastParsed, ZERO)) {
        var rootK = dswapSdkCore.sqrt(JSBI.multiply(this.reserve0.quotient, this.reserve1.quotient));
        var rootKLast = dswapSdkCore.sqrt(kLastParsed);
        if (JSBI.greaterThan(rootK, rootKLast)) {
          var numerator = JSBI.multiply(totalSupply.quotient, JSBI.subtract(rootK, rootKLast));
          var denominator = JSBI.add(JSBI.multiply(rootK, FIVE), rootKLast);
          var feeLiquidity = JSBI.divide(numerator, denominator);
          totalSupplyAdjusted = totalSupply.add(dswapSdkCore.CurrencyAmount.fromRawAmount(this.liquidityToken, feeLiquidity));
        } else {
          totalSupplyAdjusted = totalSupply;
        }
      } else {
    ...
    return dswapSdkCore.CurrencyAmount.fromRawAmount(token, JSBI.divide(JSBI.multiply(liquidity.quotient, this.reserveOf(token).quotient), totalSupplyAdjusted.quotient));
  };
  _proto.derivePercentAfterSellFees = function derivePercentAfterSellFees(inputAmount) {
    ...
    if (sellFeeBips != null && sellFeeBips.gt(bignumber.BigNumber.from(0))) {
      return ONE_HUNDRED_PERCENT.subtract(new dswapSdkCore.Percent(JSBI.BigInt(sellFeeBips)).divide(BASIS_POINTS));
    } else {
      return ZERO_PERCENT;
    }
  };
  _proto.derivePercentAfterBuyFees = function derivePercentAfterBuyFees(outputAmount) {
    ...
    if (buyFeeBps != null && buyFeeBps.gt(bignumber.BigNumber.from(0))) {
      return ONE_HUNDRED_PERCENT.subtract(new dswapSdkCore.Percent(JSBI.BigInt(buyFeeBps)).divide(BASIS_POINTS));
    } else {
      return ZERO_PERCENT;
    }
  };
  return _createClass(Pair, [{
    ...
    get: function get() {
      ...
      return new dswapSdkCore.Price(this.token0, this.token1, result.denominator, result.numerator);
    }
    /**
    ...
    get: function get() {
      ...
      return new dswapSdkCore.Price(this.token1, this.token0, result.denominator, result.numerator);
    }
  }, {
    key: "chainId",
    get: function get() {
    ...
    get: function get() {
      ...
      for (var _iterator2 = _createForOfIteratorHelperLoose(this.pairs.entries()), _step2; !(_step2 = _iterator2()).done;) {
        var _step2$value = _step2.value,
          i = _step2$value[0],
          pair = _step2$value[1];
        prices.push(this.path[i].equals(pair.token0) ? new dswapSdkCore.Price(pair.reserve0.currency, pair.reserve1.currency, pair.reserve0.quotient, pair.reserve1.quotient) : new dswapSdkCore.Price(pair.reserve1.currency, pair.reserve0.currency, pair.reserve1.quotient, pair.reserve0.quotient));
      }
      var reduced = prices.slice(1).reduce(function (accumulator, currentValue) {
        return accumulator.multiply(currentValue);
      }, prices[0]);
      return this._midPrice = new dswapSdkCore.Price(this.input, this.output, reduced.denominator, reduced.numerator);
    }
  }, {
    key: "chainId",
    get: function get() {
...
var Trade = /*#__PURE__*/function () {
  function Trade(route, amount, tradeType) {
    ...
    if (tradeType === dswapSdkCore.TradeType.EXACT_INPUT) {
      ...
      this.inputAmount = dswapSdkCore.CurrencyAmount.fromFractionalAmount(route.input, amount.numerator, amount.denominator);
      this.outputAmount = dswapSdkCore.CurrencyAmount.fromFractionalAmount(route.output, tokenAmounts[tokenAmounts.length - 1].numerator, tokenAmounts[tokenAmounts.length - 1].denominator);
    } else {
      ...
      this.inputAmount = dswapSdkCore.CurrencyAmount.fromFractionalAmount(route.input, tokenAmounts[0].numerator, tokenAmounts[0].denominator);
      this.outputAmount = dswapSdkCore.CurrencyAmount.fromFractionalAmount(route.output, amount.numerator, amount.denominator);
    }
    this.executionPrice = new dswapSdkCore.Price(this.inputAmount.currency, this.outputAmount.currency, this.inputAmount.quotient, this.outputAmount.quotient);
    this.priceImpact = dswapSdkCore.computePriceImpact(route.midPrice, this.inputAmount, this.outputAmount);
  }
  /**
  ...
  Trade.exactIn = function exactIn(route, amountIn) {
    return new Trade(route, amountIn, dswapSdkCore.TradeType.EXACT_INPUT);
  }
  /**
  ...
  Trade.exactOut = function exactOut(route, amountOut) {
    return new Trade(route, amountOut, dswapSdkCore.TradeType.EXACT_OUTPUT);
  }
  /**
  ...
  _proto.minimumAmountOut = function minimumAmountOut(slippageTolerance) {
    ...
    if (this.tradeType === dswapSdkCore.TradeType.EXACT_OUTPUT) {
      ...
      var slippageAdjustedAmountOut = new dswapSdkCore.Fraction(ONE).add(slippageTolerance).invert().multiply(this.outputAmount.quotient).quotient;
      return dswapSdkCore.CurrencyAmount.fromRawAmount(this.outputAmount.currency, slippageAdjustedAmountOut);
    }
  }
  /**
  ...
  _proto.maximumAmountIn = function maximumAmountIn(slippageTolerance) {
    ...
    if (this.tradeType === dswapSdkCore.TradeType.EXACT_INPUT) {
      ...
      var slippageAdjustedAmountIn = new dswapSdkCore.Fraction(ONE).add(slippageTolerance).multiply(this.inputAmount.quotient).quotient;
      return dswapSdkCore.CurrencyAmount.fromRawAmount(this.inputAmount.currency, slippageAdjustedAmountIn);
    }
  }
  /**
  ...
  currentPairs, nextAmountIn, bestTrades) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (amountOut.currency.equals(tokenOut)) {
        dswapSdkCore.sortedInsert(bestTrades, new Trade(new Route([].concat(currentPairs, [pair]), currencyAmountIn.currency, currencyOut), currencyAmountIn, dswapSdkCore.TradeType.EXACT_INPUT), maxNumResults, tradeComparator);
  ...
  currentPairs, nextAmountOut, bestTrades) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (amountIn.currency.equals(tokenIn)) {
        dswapSdkCore.sortedInsert(bestTrades, new Trade(new Route([pair].concat(currentPairs), currencyIn, currencyAmountOut.currency), currencyAmountOut, dswapSdkCore.TradeType.EXACT_OUTPUT), maxNumResults, tradeComparator);
      } else if (maxHops > 1 && pairs.length > 1) {
        var pairsExcludingThisPair = pairs.slice(0, i).concat(pairs.slice(i + 1, pairs.length));
...
var Router = /*#__PURE__*/function () {
  ...
  Router.swapCallParameters = function swapCallParameters(trade, options) {
    ...
    var to = dswapSdkCore.validateAndParseAddress(options.recipient);
    ...
    switch (trade.tradeType) {
      case dswapSdkCore.TradeType.EXACT_INPUT:
        if (etherIn) {
        ...
        break;
      case dswapSdkCore.TradeType.EXACT_OUTPUT:
...
//# sourceMappingURL=dswap-v2-sdk.cjs.development.js.map
